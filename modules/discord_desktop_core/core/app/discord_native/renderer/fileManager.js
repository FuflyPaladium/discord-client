...
async function readLogFiles(maxSize) {
  ...
  const voicePath = path.join(modulePath, 'discord_voice');
  const hookPath = path.join(modulePath, 'discord_hook');
  const filesToUpload = [path.join(voicePath, 'discord-webrtc_0'), path.join(voicePath, 'discord-webrtc_1'), path.join(voicePath, 'discord-last-webrtc_0'), path.join(voicePath, 'discord-last-webrtc_1'), path.join(voicePath, 'audio_state.json'), path.join(hookPath, 'hook.log')];

  for (const file of await readdir(voicePath)) {
    if (/\.(?:tsi|tsd)$/.test(file)) {
      filesToUpload.push(path.join(voicePath, file));
    }
